...
document.querySelectorAll('.close-btn, .modal-footer button[data-modal]').forEach(element => {
    element.addEventListener('click', (e) => {
        const modalId = e.currentTarget.getAttribute('data-modal') || e.currentTarget.closest('.modal').id;
...
function applyTableFilters() {
    ...
    document.querySelectorAll('#leave-table-body tr').forEach(row => {
        ...
        const employeeName = row.querySelector('[data-employee-name]').textContent.toLowerCase();
...
document.querySelectorAll('.delete-icon').forEach(icon => {
    icon.addEventListener('click', (e) => {
        ...
        const employeeName = row.querySelector('[data-employee-name]').textContent.trim();

        document.getElementById('delete-employee-name').textContent = employeeName;
        document.getElementById('confirm-delete-btn').setAttribute('data-leave-id', leaveId);
...
document.getElementById('confirm-delete-btn').addEventListener('click', (e) => {
    ...
    const rowToDelete = document.querySelector(`tr[data-leave-id="${leaveIdToDelete}"]`);
    if (rowToDelete) {
        rowToDelete.remove();
...
// updated data after input it unto form^^
    const rowToUpdate = document.querySelector(`tr[data-leave-id="${leaveIdToEdit}"]`);
    if (rowToUpdate) {
        ...
        const typeText = document.querySelector(`#edit-leave-type option[value="${newType}"]`).textContent;

        rowToUpdate.setAttribute('data-leave-type', newType);
        rowToUpdate.children[2].querySelector('[data-leave-type-text]').textContent = typeText; 
        ...
        rowToUpdate.querySelector('.info-icon').setAttribute('data-type-info', newType); 
...
const leaveInfo = {
    'Annual': {
        name: 'إجازة سنوية',
        description: 'إجازة يتمتع بها الموظف كل عام لأغراض الترفيه والاستراحة. يتم احتسابها من رصيده السنوي المخصص.',
        maxDays: '30 يومًا'
    },
    'Casual': {
        name: 'إجازة عارضة',
        description: 'إجازة قصيرة وغير مخطط لها تُمنح للموظف في حالات الطوارئ الشخصية أو الظروف المفاجئة.',
        maxDays: '7 أيام في السنة'
    },
    'Medical': {
        name: 'إجازة مرضية',
        description: 'إجازة تُمنح للموظف عند المرض، وتتطلب تقديم تقرير طبي رسمي. يتم احتسابها وفقًا للوائح الشركة.',
...
document.querySelectorAll('.info-icon').forEach(icon => {
    icon.addEventListener('click', (e) => {
        ...
        if (info) {
            ...
            document.getElementById('info-leave-description').textContent = info.description;
